...
public class ClienteController {
    ...
    private void configurarEventos() {
        view.getInserirBtn().addActionListener(e -> acaoInserir());
        view.getAlterarBtn().addActionListener(e -> acaoAlterar());
        view.getExcluirBtn().addActionListener(e -> acaoExcluir());
    }
    ...
    public final void atualizarTabela() {
        ...
        for (int i = 0; i < repository.getClientes().size(); i++) {
            ClienteModel cliente = repository.getClientes().getElementAt(i);
    ...
    private void acaoInserir() {
        ...
        dialog.getBtnConfirmar().addActionListener(e -> {
            ...
            try {
                ...
                atualizarTabela();
                JOptionPane.showMessageDialog(dialog, "Cliente cadastrado com sucesso!");
                dialog.dispose();
            } catch (Exception ex) {
    ...
    private void acaoAlterar() {
        ...
        if (linhaSelecionada == -1) {
            JOptionPane.showMessageDialog(view, "Selecione um cliente para alterar!");
            return;
        }

        int id = (Integer) view.getTabela().getValueAt(linhaSelecionada, 0);
        ...
        dialog.getBtnConfirmar().addActionListener(e -> {
            ...
            String novoCpf = dialog.getTxtCpf().getText().replaceAll("[^0-9]", "").trim();
            ...
            try {
                ...
                atualizarTabela();
                JOptionPane.showMessageDialog(dialog, "Cliente alterado com sucesso!");
                dialog.dispose();
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(dialog, ex.getMessage(), "Erro", JOptionPane.ERROR_MESSAGE);
            }
    ...
    private void acaoExcluir() {
        ...
        if (linhaSelecionada == -1) {
            JOptionPane.showMessageDialog(view, "Selecione um cliente para excluir!");
            return;
        }
